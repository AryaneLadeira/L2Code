...
const plusOrMinus = (constant) => {
  return constant > 0
    ? { constant: 1, instruction: "PLUSONE" }
    : { constant: -1, instruction: "MINUSONE" };
};
...
const isPositive = (constantCreated) => {
  return (constantCreated.actualConstant === 1 ||
    constantCreated.actualConstant % 2 !== 0) &&
    constantCreated.actualConstant * 2 <= constantCreated.constant
...
const structureFileAndSave = (verifiedConstantsObjects) => {
  const verifiedConstantsArray = verifiedConstantsObjects.map(
    (constantObject) => {
...
const verifyConstant = () => {
  ...
  for (let i = 0; i < constantsArray.length; i++) {
    ...
    if (constantsArray[i] > 32767 || constantsArray[i] < -32768) {
      console.log(`Constante ${constantsArray[i]} fora do escopo.`);
    } else {
